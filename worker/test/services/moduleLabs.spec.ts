...
import { getModuleLabs, updateModuleLab } from "../../src/services/moduleLabs";
...
describe("updateModuleLab", () => {
	it("creates a new entry when module does not exist", async () => {
		const moduleId = "uml-mod-new";
		const result = await updateModuleLab(env, moduleId, "lab-1", true);
	...
	it("adds a new lab key to an existing module", async () => {
		...
		const result = await updateModuleLab(env, moduleId, "lab-2", false);
	...
	it("updates an existing lab from true to false", async () => {
		...
		const result = await updateModuleLab(env, moduleId, "lab-1", false);
	...
	it("persists the update to KV", async () => {
		...
		await updateModuleLab(env, moduleId, "lab-x", true);
